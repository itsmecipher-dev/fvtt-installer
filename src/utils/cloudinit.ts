...
${licenseStripped ? `
  ...
  - |
    ...
    EOFLIC
` : ''}${spacesConfig ? `
  # S3-compatible storage config for Foundry (bucket already created via installer)
  - |
    cat > /home/foundry/foundrydata/Config/aws.json << EOFAWS
    {
      "buckets": ["${spacesConfig.bucket}"],
      "region": "${spacesConfig.region}",
      "endpoint": "https://${spacesConfig.region}.digitaloceanspaces.com",
      "credentials": {
...
` : ''}
  ...
  - cd /home/foundry && wget -q "${foundryDownloadUrl}" -O foundry.zip
